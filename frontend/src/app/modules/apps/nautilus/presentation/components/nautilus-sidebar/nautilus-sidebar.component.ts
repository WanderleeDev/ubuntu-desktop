...
import { NautilusSection, INautilusSection } from "../../../domain/nautilus.model";
...
@Component({
  ...
  imports: [NgClass],
  template: `
    <div class="w-64 bg-[#383838] border-r border-white/5 h-full flex flex-col overflow-y-auto custom-scrollbar">
      <div class="p-4">
        <div class="relative">
      ...
      <div class="flex-1 space-y-6 pb-6">
        @for (cat of categories; track cat.id) {
          <div>
            <h3 class="px-6 mb-2 text-[10px] font-bold uppercase tracking-wider text-white/30">{{ cat.label }}</h3>
            <div class="space-y-0.5">
              @for (section of getSectionsByCategory(cat.id); track section.id) {
                <button
                  (click)="selectSection(section.id)"
                  [ngClass]="{
                    'bg-orange-500/10 text-orange-500 font-medium': store.currentSection() === section.id,
                    'text-white/70 hover:bg-white/5': store.currentSection() !== section.id
                  }"
                ...
                >
                  @if (store.currentSection() === section.id) {
                    <div class="absolute left-0 w-1 h-6 bg-orange-500 rounded-r-full"></div>
                  }
                  <span class="material-symbols-outlined text-lg opacity-80">{{ section.icon }}</span>
...
export class NautilusSidebarComponent {
  ...
  getSectionsByCategory(category: string) {
    return this.sections.filter((s: INautilusSection) => s.category === category);
  }
